...
gt.states.total <- Databyyear %>%
  rename("Observations" = length, "Mean" = mean, "Median" = median, "SD" = sd, "Skew" = skew, "State" = state) %>% 
    gt %>% 
  fmt_number(columns = c(Observations), decimals = 0) %>%
  fmt_number(columns = c(Mean), decimals=3) %>%
  fmt_number(columns = c(SD), decimals = 3) %>%
  fmt_number(columns = c(Skew), decimals = 3) %>%
  fmt_number(columns = c(Median), decimals = 3) %>% 
  
  tab_header(
    title="Administration of In-Situ Hazard Mitigation Grants",
  ) %>% 
  ...
  tab_footnote(
    footnote="Parishes were considered the county equivalent",
    locations = cells_body(column = State, row = 4)) %>% 
  
    tab_footnote(
    footnote="Independent cities such as Hampton were considered counties for this analysis",
    locations = cells_body(column = State, row = 8))
